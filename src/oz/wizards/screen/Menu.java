...
public class Menu implements Screen, Runnable {
		...
		while(Keyboard.next()) {
			if(Keyboard.getEventKeyState() == true) {
				if(Keyboard.getEventKey() == Keyboard.KEY_UP) {
					...
					if(currentlyHighlighted < 3) currentlyHighlighted++;
				}
				if(Keyboard.getEventKey() == Keyboard.KEY_RETURN) {
					if(currentlyHighlighted == QUIT) {
						...
						Main.networkManagerThread = new Thread(Main.networkManager);
	...
	public void draw() {
		...
		shaderBase.enable();
		font.draw(new Vector2f(-ratio/2.f, 0), 0.005f, new Vector3f(0.8f, 0.8f, 0.8f), "mmaze");
		Vector3f n = new Vector3f(0.5f, 0.5f, 0.5f);
		Vector3f h = new Vector3f(1.f, 1.f, 1.f);
		font.draw(new Vector2f(-ratio/2.f, -0.1f), 0.003f, (currentlyHighlighted == PLAY_MULTIPLAYER ? h :n), "multiplayer");
		font.draw(new Vector2f(-ratio/2.f, -0.15f), 0.003f, (currentlyHighlighted == PLAY_SINGLEPLAYER ? h :n), "singleplayer");
		font.draw(new Vector2f(-ratio/2.f, -0.20f), 0.003f, (currentlyHighlighted == SET_OPTIONS ? h :n), "options");
		font.draw(new Vector2f(-ratio/2.f, -0.25f), 0.003f, (currentlyHighlighted == QUIT ? h :n), "quit");
		vb.render();
		shaderBase.disable();
